...
import { fetchGuildRankList, fetchTrophyRankList, searchLatestPage } from "./ms2fetch.ts"

test.skip("길드 목록 파싱1 (1페이지)", async () => {
...
test.skip("길드 목록 파싱1 (8페이지)", async () => {
...
test.skip("길드 목록 파싱2 (10만페이지)", async () => {
...
test("트로피 목록 마지막 체크", async () => {
  const latestPage = await searchLatestPage((page) => {
    return fetchTrophyRankList(page)
  })
  console.log(latestPage)
